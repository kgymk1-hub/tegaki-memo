# tegaki-memo（手書きメモ）

ブラウザだけで動く、シンプルな手書きメモ・お絵かきアプリです。HTML / CSS / JavaScript の静的ファイルだけで構成しているため、GitHub Pages で公開できます。スマホ・タブレット・PC から、指・タッチペン・マウスでメモやラフスケッチを作成できます。

## GitHub Pages URL

```text
https://kgymk1-hub.github.io/tegaki-memo/
```

## 実装済み機能

- canvas への手書き描画
- ペン / 半透明マーカー / 消しゴム
- 色変更（プリセット色・任意色）
- 太さ変更
- 図形・注釈ツール
  - 直線（選択後、キャンバス上でドラッグして描画）
  - 四角形（選択後、キャンバス上でドラッグして描画）
  - 円 / 楕円（選択後、キャンバス上でドラッグして描画）
  - 矢印（選択後、キャンバス上でドラッグして描画）
  - 点線（選択後、キャンバス上でドラッグして描画）
  - テキスト入力（選択後、文字を入れたい位置をタップ。入力後・キャンセル後・空欄入力後は自動でペンへ戻る）
- 矩形選択 / コピー / 切り取り / 貼り付け
  - 選択範囲は現在選択中の1レイヤーのみ対象
  - 選択範囲は点線枠で表示
  - 選択範囲をアプリ内部クリップボードへコピー可能
  - 選択範囲を切り取り、範囲を透明化可能
  - 内部クリップボードから現在レイヤーへ貼り付け可能
  - 貼り付け後はプレビューをドラッグ移動し、「確定」で現在レイヤーへ反映、「取消」で破棄
  - 選択状態と内部クリップボードは `.tegaki` 保存対象外
  - 自由選択、全レイヤー横断選択、OSクリップボード連携、選択範囲の拡大縮小 / 回転は未対応
- Undo
  - 描画（ペン / マーカー / 図形 / テキスト）
  - 現在レイヤーの消去
  - 画像読込
  - レイヤー追加 / 複製 / 削除 / 名前変更 / 表示・非表示 / 順序変更 / 下へ結合
  - 背景モード変更 / 背景色変更
  - レイヤー透明度変更
  - 選択範囲の切り取り / 貼り付け確定
- 背景設定
  - 白
  - 任意色
  - 透明（画面上はチェック柄）
- 画像読込
  - 新規レイヤーとして読み込み
  - 現在レイヤーへ読み込み
  - 読込直後に中央配置のプレビューを表示
  - 配置中はドラッグで位置調整
  - 配置中に倍率スライダーで10%〜300%の範囲で拡大縮小（縦横比維持）
  - 配置中に左90° / 右90°ボタンで90度単位の回転
  - 倍率変更と回転は確定前のプレビューに反映
  - 配置中に画面サイズが変わった場合は、回転状態を維持し、倍率を100%に戻して新しいキャンバス内に収まるよう再中央配置
  - 「確定」で現在の位置・倍率・回転をレイヤーへ反映、「取消」で読込をキャンセル
  - 画像の確定はUndo履歴に1回分として記録され、取消やドラッグ・倍率変更・回転ではUndo履歴を増やさない
  - 配置中は描画ツールを使えず、通常描画やPNG保存の前に確定 / 取消が必要
- PNG保存
  - 表示中レイヤーのみを書き出し
  - 背景設定とレイヤー透明度を反映
- レイヤー管理
  - 最大5枚
  - 非表示レイヤーには描画・文字入力・現在レイヤーへの画像読込はできず、操作時に案内を表示
  - 追加 / 複製 / 削除
  - 複製は表示中レイヤーのみ可能（非表示レイヤーは複製不可）
  - レイヤー名変更
  - 表示 / 非表示
  - 上下移動
  - 下へ結合
  - 透明度調整
- 自動保存 / 前回作業復元
  - 描画、背景、レイヤー操作、画像確定、Undoなどの変更後にブラウザ内へ自動保存
  - 起動時に前回作業データがある場合は復元確認を表示
  - ブラウザを閉じた後やページ再読み込み後も前回作業を復元可能
  - 自動保存データ削除ボタンでブラウザ内の前回作業データのみ削除可能
  - 自動保存は同じ端末・同じブラウザのサイトデータ内に保存されるため、端末やブラウザを変えると復元されません
  - ブラウザのサイトデータ削除やキャッシュ削除により自動保存データも消える場合があります
- 独自形式 `.tegaki` 保存 / 読込
  - `.tegaki` は Tegaki Memo Project として保存する再編集用のプロジェクトファイルです
  - 「作業保存」でレイヤー構造、レイヤー名、表示 / 非表示、透明度、背景設定、アクティブレイヤーなどを保持して保存します
  - 「作業読込」で `.tegaki` またはJSONファイルを読み込み、現在の作業内容を置き換えて復元できます
  - `.tegaki` の中身はJSON形式です
  - レイヤー画像を含むため、キャンバス内容や画像、レイヤー数によってファイルサイズが大きくなる場合があります
  - 自動保存はブラウザ内保存、`.tegaki` は端末へダウンロードするファイル保存です
- PWA対応
  - `manifest.json`
  - `service-worker.js`
  - ホーム画面追加対応
  - アプリシェルのキャッシュ対応

## 使い方

1. GitHub Pages URL、またはローカルの `index.html` をブラウザで開きます。
2. キャンバス上を指・タッチペン・マウスでなぞると描画できます。
3. ツールバーでペン、マーカー、消しゴム、図形、文字、色、太さ、背景、レイヤーを切り替えます。
4. 直線・四角形・円 / 楕円・矢印・点線は、ツールを選んだあとキャンバス上でドラッグして描画します。
5. 文字ツールは、ツールを選んだあと文字を入れたい位置をタップして入力します。入力後、キャンセル後、空欄入力後はいずれも自動でペンへ戻ります。
6. 「選択レイヤー消去」はアプリ全体ではなく、選択中レイヤーだけを消去します。
7. 非表示レイヤーには描画できません。非表示レイヤーを選択中に描画・文字入力・現在レイヤーへの画像読込をしようとすると、表示に切り替えるか別レイヤーを選ぶよう案内されます。
8. 非表示レイヤーは複製できません。表示中のレイヤーを選択してから複製してください。
9. 「画像読込先」で画像を「新規レイヤー」または「現在レイヤー」に読み込むか選び、「画像読込」ボタンから画像ファイルを選択します。
10. 画像を読み込むとキャンバス中央にプレビュー表示されます。ドラッグで位置を調整し、画像倍率スライダーで10%〜300%の拡大縮小、左90° / 右90°ボタンで90度単位の回転ができます。
11. 倍率変更と回転は確定前のプレビューに反映され、「確定」で現在の位置・倍率・回転をレイヤーへ描画します。「取消」すると読込前の状態に戻り、確定後はUndoで画像読込前へ戻せます。
12. 画像配置中はペン・マーカー・消しゴム・図形・文字などの描画ツールは使えません。PNG保存や再度の画像読込を行う前に、画像を確定または取消してください。画面サイズが変わった場合、配置中の画像は回転状態を維持し、倍率を100%に戻して新しいキャンバス内に収まるよう再中央配置されます。
13. 「選択」ツールでキャンバス上をドラッグすると、現在レイヤー上に矩形選択を作成できます。選択後は「コピー」「切取」「貼付」「解除」を使用できます。
14. コピーはレイヤー内容を変更せず、切り取りは選択範囲を透明にしてUndo対象になります。貼り付けは現在レイヤーへのプレビューとして表示され、ドラッグで位置調整してから「確定」で反映、「取消」で破棄できます。
15. 選択状態と内部クリップボードはブラウザ内の一時状態で、`.tegaki` ファイルには保存されません。自由選択、OSクリップボード連携、全レイヤー選択、選択範囲の拡大縮小 / 回転は未対応です。
16. レイヤーの「選択レイヤー透明度」スライダーで選択中レイヤーの見え方を調整できます。
17. 「戻す」で直前の操作をUndoできます。
18. 「PNG保存」で現在の表示内容を完成画像として保存できます。
19. 「作業保存」で、後から再編集できる `.tegaki` プロジェクトファイルを保存できます。PNGとは異なり、レイヤーや背景設定を保持します。
20. 「作業読込」で `.tegaki` ファイルを読み込めます。読込前に確認が表示され、OKすると現在の作業内容はファイルの内容に置き換わります。
21. 作業内容は変更後に自動保存されます。次回起動時に「前回の作業データがあります。復元しますか？」と表示された場合、OKで復元、キャンセルで新規状態として開始できます。
22. 「自動保存削除」は、現在のキャンバス内容を消さずにブラウザ内の前回作業データだけを削除します。

## ファイル構成

```text
tegaki-memo/
├─ index.html         # 画面構造、PWA manifest / theme-color 読み込み
├─ style.css          # レイアウト・見た目・スマホ向け調整
├─ manifest.json      # PWA設定
├─ service-worker.js  # キャッシュ制御
├─ icons/
│  └─ icon.svg        # PWAアイコン
└─ js/
   ├─ state.js             # DOM参照、共有状態、定数
   ├─ utils.js             # ポインター座標、スナップショットなどの汎用処理
   ├─ history.js           # Undo履歴の保存・復元
   ├─ layers.js            # レイヤー作成・追加・削除・複製・結合・透明度
   ├─ canvas-render.js     # canvas描画、合成、リサイズ、背景、PNG保存
   ├─ drawing-tools.js     # ペン、マーカー、消しゴム、図形、文字
   ├─ image-placement.js   # 画像読込後の配置調整、拡大縮小、90°回転、確定 / 取消
   ├─ selection-tools.js    # 矩形選択、コピー、切り取り、貼り付け
   ├─ project-storage.js   # 作業状態のJSON化、自動保存、前回作業復元、削除
   ├─ ui.js                # UI表示更新、ヒント、イベント登録
   ├─ pwa.js               # Service Worker登録
   └─ app.js               # 起動時の初期化順序
```

`index.html` では ES Modules を使わず、従来どおりグローバルスコープの JavaScript を `defer` 付きで次の順に読み込みます。

```html
<script src="js/state.js?v=17" defer></script>
<script src="js/utils.js?v=17" defer></script>
<script src="js/history.js?v=17" defer></script>
<script src="js/layers.js?v=17" defer></script>
<script src="js/canvas-render.js?v=17" defer></script>
<script src="js/drawing-tools.js?v=17" defer></script>
<script src="js/image-placement.js?v=17" defer></script>
<script src="js/selection-tools.js?v=17" defer></script>
<script src="js/project-storage.js?v=17" defer></script>
<script src="js/ui.js?v=17" defer></script>
<script src="js/pwa.js?v=17" defer></script>
<script src="js/app.js?v=17" defer></script>
```

## PWA更新時の注意

JavaScript / CSSを追加・変更した場合は、古いファイルがService Workerキャッシュに残らないように、以下を同じ版へ更新してください。

- `index.html` の読み込みバージョン（例: `?v=17`）
- `service-worker.js` の `CACHE_NAME`
- `service-worker.js` の `APP_SHELL` に含めるファイル一覧とクエリ文字列

特にJavaScriptファイルを追加した場合は、`index.html` の `<script>` と `APP_SHELL` の両方へ追加してください。今回追加した `js/selection-tools.js?v=17` もキャッシュ対象です。


## PWAとしての使い方

- GitHub Pages URLをブラウザで開き、ブラウザの「ホーム画面に追加」または「アプリをインストール」から追加できます。
- `manifest.json` と `service-worker.js` は相対パスで読み込むため、GitHub Pages のサブディレクトリ配信でも動作します。
- service worker はアプリシェル（HTML / CSS / JavaScript / manifest / SVGアイコン）をキャッシュするため、一度開いた後はオフラインでも起動しやすくなります。
- 更新反映を安定させるため、HTML / CSS / JavaScript はネットワーク優先で取得し、取得に成功した場合はキャッシュを更新します。ネットワークに接続できない場合のみキャッシュを利用します。
- PWAやGitHub Pagesでは、デプロイ後もしばらく古いHTML / CSS / JavaScriptがキャッシュから表示される場合があります。

### 更新が反映されない場合

GitHub PagesやPWAでは、Service Workerやブラウザキャッシュにより古い `js/` 配下のJavaScript / `style.css` / `index.html` が残る場合があります。更新が反映されない場合は、以下を試してください。

1. ページを数回再読み込みする
2. Chromeのサイトデータ / キャッシュを削除する
3. ホーム画面に追加したPWAを一度削除する
4. `service-worker.js` の `CACHE_NAME` が更新されているか確認する
5. 再度GitHub Pagesを開き、必要に応じてホーム画面に追加し直す

開発者向けの注意: `js/` 配下のJavaScript / `style.css` / `index.html` を変更した場合は、PWA更新のために `index.html` の読み込みバージョン、`service-worker.js` の `CACHE_NAME`、`APP_SHELL` を同じ版にそろえて更新してください。

## 動作確認チェックリスト

### コード整形

- [ ] `js/` 配下のJavaScriptと `style.css` が読みやすい改行・インデントになっている
- [ ] ブラウザのコンソールに構文エラーが出ない
- [ ] ペン・マーカー・消しゴム・色・太さ変更が動作する
- [ ] 非表示レイヤーには描画できず、画面上のヒント「非表示レイヤーには描画できません。表示に切り替えるか、別のレイヤーを選択してください。」で案内される
- [ ] 非表示レイヤー選択中にキャンバスを触ってもalertが繰り返し表示されず、レイヤー選択・表示切替・他ボタン操作を続けられる
- [ ] 非表示レイヤー選択中は描画できず、不要なUndo履歴が増えない

### 自動保存 / 前回作業復元

- [ ] ペン、マーカー、消しゴムで描いた後に自動保存される
- [ ] 直線 / 四角形 / 円・楕円 / 矢印 / 点線を描いた後に自動保存される
- [ ] 文字入力後に自動保存され、文字ツールはペンへ戻る
- [ ] 背景モード / 背景色変更後に自動保存される
- [ ] レイヤー追加 / 削除 / 複製 / 名前変更 / 表示・非表示 / 順序変更 / 下へ結合 / 透明度変更 / 消去後に自動保存される
- [ ] 画像確定後に自動保存される
- [ ] 選択範囲の切り取り後に自動保存される
- [ ] 貼り付け確定後に自動保存される
- [ ] Undo後に自動保存される
- [ ] 画像配置中のドラッグ移動だけでは不要な自動保存が連発しない
- [ ] ページ再読み込み後に前回作業データの復元確認が表示される
- [ ] OKでレイヤー名、表示状態、透明度、背景、アクティブレイヤー、描画内容が復元される
- [ ] キャンセルで新規状態として開始できる
- [ ] 復元後にそのまま描画できる
- [ ] localStorage容量超過などで保存に失敗してもアプリ操作は止まらず、ステータスとconsoleで確認できる

#### 自動保存 / `.tegaki` の注意点

現在の自動保存は `localStorage` を使用しています。レイヤー画像をData URLとして保存するため、キャンバスサイズやレイヤー数、画像内容によってはブラウザの容量制限により保存できない場合があります。`.tegaki` ファイルも同じくレイヤー画像をJSON内に含むため、画像やレイヤーを多く含む場合はファイルサイズが大きくなることがあります。将来的には、より大きなデータを扱いやすい `IndexedDB` 方式への移行を検討しています。

### PWA

- [ ] `manifest.json` が読み込まれる
- [ ] `service-worker.js` が登録される
- [ ] GitHub Pages上でホーム画面追加の候補として表示される
- [ ] `CACHE_NAME` を変更するとキャッシュ更新を管理できる
- [ ] HTML / CSS / JavaScript の更新時はネットワーク優先で新しいファイルが取得される

### Undo / レイヤー

- [ ] 直線を描ける
- [ ] 四角形を描ける
- [ ] 円 / 楕円を描ける
- [ ] 矢印を描ける
- [ ] 点線を描ける
- [ ] 文字を入力でき、入力後・キャンセル後・空欄入力後にペンへ戻る
- [ ] 選択ツールで矩形選択でき、短すぎるドラッグでは選択されない
- [ ] 選択範囲をコピーでき、コピーだけではUndo履歴やレイヤー内容が変わらない
- [ ] 選択範囲を切り取りでき、Undoで戻せる
- [ ] コピーまたは切り取り後に貼り付けでき、プレビューをドラッグ移動して確定 / 取消できる
- [ ] 貼り付け確定時に現在レイヤーはクリアされず、既存内容の上へ描画される
- [ ] 画像配置中や非表示レイヤーでは選択・コピー・切取・貼付ができない
- [ ] レイヤー切替や `.tegaki` 読込後に選択状態が残らない

- [ ] 描画、図形、テキスト入力をUndoできる
- [ ] 表示中レイヤーを複製できる
- [ ] 非表示レイヤーでは複製ボタンが無効になる
- [ ] 最大レイヤー数に達すると複製ボタンが無効になる
- [ ] 複製後、複製レイヤーがアクティブになる
- [ ] レイヤー追加 / 複製をUndoできる
- [ ] レイヤー消去が選択中レイヤーだけに効く
- [ ] レイヤー表示 / 非表示をUndoできる
- [ ] 背景モード変更をUndoできる
- [ ] 背景色変更をUndoできる
- [ ] 透明度変更をUndoできる

### 下へ結合

- [ ] 表示中のアクティブレイヤーを表示中の下レイヤーへ結合できる
- [ ] 非表示のアクティブレイヤーは結合できず、理由が表示される
- [ ] 結合先の下レイヤーが非表示の場合は結合できず、理由が表示される
- [ ] 透明度付きレイヤーは見た目どおりに結合される

### 画像読込 / PNG保存 / `.tegaki` 保存読込

- [ ] 画像を読み込むと中央にプレビュー表示され、まだレイヤーへ確定されていない
- [ ] 画像配置中にドラッグで移動できる
- [ ] 倍率スライダーで拡大縮小でき、倍率表示が更新される
- [ ] 左90° / 右90°ボタンで90度単位に回転できる
- [ ] 回転後もドラッグ移動・拡大縮小できる
- [ ] 「確定」で新規レイヤーとして画像を配置できる
- [ ] 「確定」で現在レイヤーへ画像を配置できる
- [ ] 「取消」でレイヤーを変更せず画像読込を破棄できる
- [ ] 確定後にUndoすると画像読込前へ戻る
- [ ] 取消後はUndo履歴が増えない
- [ ] 画像配置中はペン・図形・文字入力ができない
- [ ] 画像配置中にPNG保存、作業保存、作業読込、再度の画像読込を行うと、確定 / 取消を促される
- [ ] PNG保存に背景設定が反映される
- [ ] PNG保存にレイヤー透明度が反映される
- [ ] 作業保存で日時入りファイル名の `.tegaki` がダウンロードされる
- [ ] `.tegaki` のJSONにレイヤー情報と背景設定が含まれる
- [ ] 作業読込で置き換え確認後、レイヤー名、表示状態、透明度、背景、アクティブレイヤーが復元される
- [ ] 作業読込後にUndo履歴が初期化され、同じファイルを再選択できる
- [ ] 不正なJSONや必須項目が不足したファイルではエラー表示される

## 実装方針メモ

- 既存の操作感を大きく変えず、ツールバーは機能ごとに行を分けています。
- Undo履歴にはレイヤー状態、アクティブレイヤー、ヒント表示状態、背景設定を保存しています。
- 「選択レイヤー消去」は選択中レイヤーだけを消去する操作として扱い、表示中レイヤーに描画が残っている場合は初期ヒントを再表示しないようにしています。
- 描画可否判定は共通化し、非表示レイヤー選択中の描画・文字入力・現在レイヤーへの画像読込では案内を表示して、不要なUndo履歴を積まないようにしています。
- ヒント表示はツール種別とアクティブレイヤーの表示状態に合わせ、図形はドラッグ、文字はタップ、非表示レイヤーは描画不可が伝わるようにしています。
- 下へ結合は非表示レイヤーが絡む場合に禁止し、見た目と結果がずれないようにしています。
- 画像読込は「新規レイヤー」を初期値にしつつ、「現在レイヤー」へ読み込む選択肢も残しています。
- 画像読込後はすぐにレイヤーへ焼き付けず、表示用canvas上のプレビューとして配置調整します。ドラッグ移動、10%〜300%の倍率変更、左90° / 右90°の90度単位回転は確定前だけ行えます。確定時だけUndo履歴を1回積み、取消時やドラッグ移動・倍率変更・回転中は履歴を増やしません。
- 選択範囲は現在レイヤーのみ対象とし、自由選択・全レイヤー横断選択・OSクリップボード連携・選択範囲の拡大縮小 / 回転は今後の予定です。
- 画像配置中は通常描画・PNG保存・作業保存・作業読込・再度の画像読込を止め、確定または取消を促します。
- `.tegaki` 保存 / 読込は `serializeProjectState()` と `restoreProjectState()` を流用し、ファイル読込後はUndo履歴をリセットして自動保存データを読込後の状態へ更新します。

## GitHub リポジトリ設定案

### About欄 Description案

```text
ブラウザで動くシンプルな手書きメモ・お絵かきPWA
```

### Topics案

```text
canvas, drawing-app, memo, handwriting, javascript, html, css, pwa, github-pages
```

## 今後の予定

- 確定後の画像レイヤーの再調整
- 任意角度での画像回転
- 自由選択 / OSクリップボード連携
- 選択範囲の拡大縮小 / 回転
- ブラシ種類の追加
- レイヤー数上限や履歴数上限の見直し
- キャンバスサイズ変更
- UI折りたたみ
- PWAアイコンのPNGサイズ別追加

## ライセンス候補

現時点では未設定です。用途に応じて以下を検討できます。

- **MIT License**: 個人制作・学習用・再利用しやすさを重視する場合に扱いやすい候補です。
- **Apache License 2.0**: 特許条項も含めて明確にしたい場合の候補です。
- **GPLv3**: 派生物にも同じライセンスでの公開を求めたい場合の候補です。

迷う場合は、シンプルで採用例が多い **MIT License** が候補になります。
